...
const PlaybackTimeline = memo(({
  ...
  return (
    <div className="absolute bottom-6 left-1/2 -translate-x-1/2 w-[95%] max-w-5xl bg-background/70 backdrop-blur-xl border border-white/20 dark:border-white/10 rounded-2xl shadow-2xl p-3 z-10 flex items-center gap-4 transition-all">
      ...
      <div className="flex items-center gap-4 pr-2">
        ...
        <div className="text-center w-16">
          ...
          <p className="text-lg font-bold">{(currentData?.distance || 0).toFixed(2)}</p>
          <p className="text-xs text-muted-foreground -mt-1">km</p>
        </div>
        <div className="text-center w-16">
          <p className="text-xs text-muted-foreground">Fuel</p>
          <p className="text-lg font-bold">{(currentData?.fuel || 0).toFixed(2)}</p>
...
PlaybackTimeline.displayName = 'PlaybackTimeline';
