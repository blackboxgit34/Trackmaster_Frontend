...
import { engineTempData, vehicles } from '@/data/mockData';
import {
  ArrowUp,
  ArrowDown,
  ChevronLeft,
...
type ReportData = (typeof engineTempData)[0];
...
const headers: { key: ReportDataKey; label: string }[] = [
  ...
  { key: 'engineTemp', label: 'Avg. Temp (°C)' },
];
...
const EngineTempReportTable = () => {
  ...
  const dailyAggregatedData = useMemo(() => {
    let filteredByDate = engineTempData;
    if (date?.from) {
      const start = startOfDay(date.from);
      const end = date.to ? endOfDay(date.to) : endOfDay(date.from);
      filteredByDate = engineTempData.filter(item => {
    ...
    return Array.from(dailyData.values()).map((dayEntries) => {
      const totalTemp = dayEntries.reduce(
        (sum, entry) => sum + entry.engineTemp,
        0
      );
      ...
      if (details.length > 0) {
        details.forEach(detail => {
          dataToExport.push({
            ...
            'Daily Avg Temp (°C)': row.engineTemp.toFixed(1),
            'Log Time': detail.dateTime.split(' ')[1],
            'Logged Temp (°C)': detail.engineTemp.toFixed(1),
  ...
  const handleExportPDF = () => {
    ...
    doc.text("Engine Temperature Report with Detailed Logs", 14, 15);
    ...
    doc.save(`engine-temp-detailed-${new Date().toISOString().split('T')[0]}.pdf`);
  };
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute('download', `engine-temp-detailed-${new Date().toISOString().split('T')[0]}.csv`);
    ...
    <Card className="shadow-sm overflow-hidden">
      <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 px-6 py-4">
        <div>
          ...
          <CardDescription>
            Overview of vehicle engine temperatures.
          </CardDescription>
        </div>
                ...
                const maxTempReading =
                  highTempDetails.length > 0
                    ? highTempDetails.reduce(
                        (max, current) =>
                          current.engineTemp > max.engineTemp ? current : max,
                        highTempDetails[0]
                      )
                    : null;
                ...
                const minTempReading =
                  lowTempDetails.length > 0
                    ? lowTempDetails.reduce(
                        (min, current) =>
                          current.engineTemp < min.engineTemp ? current : min,
                        lowTempDetails[0]
                      )
                    : null;
                                  ...
                                  <div className="grid grid-cols-2 gap-4">
                                    <div className="bg-card rounded-lg p-4 shadow-sm">
                                      ...
                                      <p className="text-2xl font-bold text-red-500">
                                        {maxTempReading?.engineTemp.toFixed(1) ?? 'N/A'} °C
                                      </p>
                                    </div>
                                    <div className="bg-card rounded-lg p-4 shadow-sm">
                                      <div className="flex justify-between items-start mb-2">
                                      ...
                                      <p className="text-2xl font-bold text-blue-500">
                                        {minTempReading?.engineTemp.toFixed(1) ?? 'N/A'} °C
                                      </p>
                                    </div>
                                  </div>
                                  <div className="bg-card rounded-lg p-5 shadow-sm">
                                      ...
                                      <Table>
                                        <TableHeader>
                                          <TableRow>
                                            ...
                                            <SortableHeader onClick={() => handleDetailsSort('engineTemp')} isSorted={detailsSortConfig.key === 'engineTemp'} sortDirection={detailsSortConfig.direction}>Temperature</SortableHeader>
                                          ...
                                          {sortedDetails.length > 0 ? (
                                            sortedDetails.map((detail) => (
                                              <TableRow key={detail.id}>
                                                ...
                                                <TableCell className="font-mono text-sm">{detail.engineTemp.toFixed(1)} °C</TableCell>
                                                <TableCell>
      ...
      <CardFooter className="flex items-center justify-between py-3 px-6 border-t bg-card">
        <div className="flex items-center gap-2">
          ...
          >
            <SelectTrigger className="w-20 h-9 text-sm focus:ring-2 focus:ring-ring">
...
export default EngineTempReportTable;