        ...
        <div className="grid grid-cols-1 gap-1.5">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold">
              {data.idlingHours.toFixed(1)} hrs
            </span>
          </div>
        </div>
      </div>
...
const CustomLegend = ({
  ...
  data: { vehicle: string; idlingHours: number }[];
}) => {
  if (!data || data.length === 0) return null;

  const avgIdlingHours =
    data.reduce((acc, curr) => acc + curr.idlingHours, 0) /
    data.length;
...
const IdlingDuration = ({ data }: Props) => {
  ...
  const chartData = useMemo(() => {
    ...
    const formattedData = data.map((item) => ({
      ...
      idlingHours: parseFloat(item.totalIdlingHours)/3600 || 0,
    }));

    if (sortOrder === 'asc') {
      return [...formattedData].sort(
        (a, b) => a.idlingHours - b.idlingHours
      );
    }

    if (sortOrder === 'desc') {
      return [...formattedData].sort(
        (a, b) => b.idlingHours - a.idlingHours
      );
    }
            ...
            <div className="flex items-center gap-2">
              ...
              <TooltipProvider>
                <UITooltip delayDuration={100}>
                  ...
                  <TooltipContent>
                    <p>
                      Total idling duration for each vehicle.
                    </p>
                  </TooltipContent>
                </UITooltip>
              </TooltipProvider>
